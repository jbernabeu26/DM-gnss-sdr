/*!
 * \file pcps_noncoherent_iq_acquisition_caf_cc.cc
...
#include "pcps_noncoherent_iq_acquisition_caf_cc.h"
...
pcps_noncoherent_iq_acquisition_caf_cc_sptr
pcps_noncoherent_iq_make_acquisition_caf_cc(const Acq_Conf &conf_)
{
    return pcps_noncoherent_iq_acquisition_caf_cc_sptr(
        new pcps_noncoherent_iq_acquisition_caf_cc(conf_));
}


pcps_noncoherent_iq_acquisition_caf_cc::pcps_noncoherent_iq_acquisition_caf_cc(
    const Acq_Conf &conf_) : gr::block("pcps_noncoherent_iq_acquisition_caf_cc",
                                 ...
                                 gr::io_signature::make(0, 0, conf_.it_size))
{
    ...
    d_consumed_samples = acq_parameters.sampled_ms * acq_parameters.samples_per_ms * (acq_parameters.zero_padding_flag ? 2 : 1);
    ...
    if (acq_parameters.zero_padding_flag)
        {
            d_fft_size = d_consumed_samples * 2;
    ...
    d_fft_code_i = std::vector<std::complex<float>>(d_fft_size);
...
pcps_noncoherent_iq_acquisition_caf_cc::~pcps_noncoherent_iq_acquisition_caf_cc() = default;


void pcps_noncoherent_iq_acquisition_caf_cc::set_resampler_latency(uint32_t latency_samples)
{
...
void pcps_noncoherent_iq_acquisition_caf_cc::set_local_code(std::complex<float> *codeI, std::complex<float> *codeQ)
{
    ...
    if (acq_parameters.acq_iq_flag)
        {
            ...
            memcpy(d_fft_if->get_inbuf(), codeI, sizeof(gr_complex) * d_consumed_samples);
            d_fft_if->execute();  // We need the FFT of local code
            volk_32fc_conjugate_32fc(d_fft_code_i, d_fft_if->get_outbuf(), d_fft_size);

            // Processing pilot component
            memcpy(d_fft_if->get_inbuf(), codeQ, sizeof(gr_complex) * d_consumed_samples);
            d_fft_if->execute();  // We need the FFT of local code
            volk_32fc_conjugate_32fc(d_fft_code_q, d_fft_if->get_outbuf(), d_fft_size);
        }
    else if (acq_parameters.acq_pilot_flag)
        {
            // Processing pilot component
            memcpy(d_fft_if->get_inbuf(), codeQ, sizeof(gr_complex) * d_consumed_samples);
            d_fft_if->execute();  // We need the FFT of local code
...
bool pcps_noncoherent_iq_acquisition_caf_cc::is_fdma()
{
...
void pcps_noncoherent_iq_acquisition_caf_cc::update_local_carrier(gsl::span<gr_complex> carrier_vector, float freq)
{
...
void pcps_noncoherent_iq_acquisition_caf_cc::init()
{
    ...
    if (d_magnitude_grid_i.empty())
        {
            d_magnitude_grid_i = std::vector<std::vector<float>>(d_num_doppler_bins, std::vector<float>(d_fft_size));
        }
    ...
    for (uint32_t doppler_index = 0; doppler_index < d_num_doppler_bins; doppler_index++)
        {
            for (uint32_t k = 0; k < d_fft_size; k++)
                {
                    d_magnitude_grid_i_i[doppler_index][k] = 0.0;
                }
...
void pcps_noncoherent_iq_acquisition_caf_cc::update_grid_doppler_wipeoffs()
{
    for (uint32_t doppler_index = 0; doppler_index < d_num_doppler_bins; doppler_index++)
        {
            int32_t doppler = -static_cast<int32_t>(acq_parameters.doppler_max) + d_doppler_step * doppler_index;
...
void pcps_noncoherent_iq_acquisition_caf_cc::update_grid_doppler_wipeoffs_step2()
{
...
void pcps_noncoherent_iq_acquisition_caf_cc::set_state(int32_t state)
{
    gr::thread::scoped_lock lock(d_setlock);  // require mutex with work function called by the scheduler
    d_state = state;
    if (d_state == 1)
...
void pcps_noncoherent_iq_acquisition_caf_cc::send_positive_acquisition()
{
...
void pcps_noncoherent_iq_acquisition_caf_cc::send_negative_acquisition()
{
...
void pcps_noncoherent_iq_acquisition_caf_cc::dump_results(int32_t effective_fft_size)
{
    d_dump_number++;
...
float pcps_noncoherent_iq_acquisition_caf_cc::max_to_input_power_statistic(uint32_t &indext, int32_t &doppler, float input_power, uint32_t num_doppler_bins, int32_t doppler_max, int32_t doppler_step)
{
    ...
    for (uint32_t i = 0; i < num_doppler_bins; i++)
        {
            if (acq_parameters.acq_iq_flag)
                {
                    // Perform operations in data component
                    volk_gnsssdr_32f_index_max_32u(&tmp_intex_t, d_magnitude_grid_i_i[i].data(), d_fft_size);
                    if (d_magnitude_grid_i_i[i][tmp_intex_t] > grid_maximum)
                        {
                            grid_maximum = d_magnitude_grid_i_i[i][tmp_intex_t];
                            index_doppler = i;
            ...
            if (acq_parameters.acq_pilot_flag)
                {
                    // Perform operations in pilot component
                    volk_gnsssdr_32f_index_max_32u(&tmp_intex_t, d_magnitude_grid_i_q[i].data(), d_fft_size);
                    if (d_magnitude_grid_i_q[i][tmp_intex_t] > grid_maximum)
                        {
                            grid_maximum = d_magnitude_grid_i_i[i][tmp_intex_t];
                            index_doppler = i;
                            index_time = tmp_intex_t;
...
float pcps_noncoherent_iq_acquisition_caf_cc::first_vs_second_peak_statistic(uint32_t &indext, int32_t &doppler, uint32_t num_doppler_bins, int32_t doppler_max, int32_t doppler_step)
{
    ...
    for (uint32_t i = 0; i < num_doppler_bins; i++)
        {
            volk_gnsssdr_32f_index_max_32u(&tmp_intex_t, d_magnitude_grid_i[i].data(), d_fft_size);
            if (d_magnitude_grid_i[i][tmp_intex_t] > firstPeak)
                {
                    firstPeak = d_magnitude_grid_i[i][tmp_intex_t];
                    index_doppler = i;
                    index_time = tmp_intex_t;
                }
        }
    ...
    memcpy(d_tmp_buffer.data(), d_magnitude_grid_i[index_doppler].data(), d_fft_size);
    do
        {
            ...
            for (uint32_t doppler_index = 0; doppler_index < d_num_doppler_bins; doppler_index++)
                {
                    ...
                    if (acq_parameters.acq_iq_flag)
                        {
                            // Processing data component
                            volk_32fc_x2_multiply_32fc(d_ifft->get_inbuf(), d_fft_if->get_outbuf(), d_fft_code_i.data(), d_fft_size);
                            d_ifft->execute();
                            volk_32fc_magnitude_squared_32f(magnitude_i, d_ifft->get_outbuf() + offset, effective_fft_size);

                            // Processing pilot component
                            volk_32fc_x2_multiply_32fc(d_ifft->get_inbuf(), d_fft_if->get_outbuf(), d_fft_code_q.data(), d_fft_size);
                            d_ifft->execute();
                            volk_32fc_magnitude_squared_32f(magnitude_q, d_ifft->get_outbuf() + offset, effective_fft_size);
                        }
                    else if (acq_parameters.acq_pilot_flag)
                        {
                            // Processing pilot component
                    ...
                    // Save computed magnitude non coherently to grid
                    d_magnitude_grid[doppler_index] = magnitude_i + magnitude_q;

                    // Record results to file if required
                    if (d_dump and d_channel == d_dump_channel)
                        {
                            memcpy(grid_.colptr(doppler_index), d_magnitude_grid_i[doppler_index].data(), sizeof(float) * effective_fft_size);
                        }
                }
            ...
            for (uint32_t doppler_index = 0; doppler_index < d_num_doppler_bins_step2; doppler_index++)
                {
                    ...
                    if (d_dump and d_channel == d_dump_channel)
                        {
                            memcpy(narrow_grid_.colptr(doppler_index), d_magnitude_grid_i[doppler_index].data(), sizeof(float) * effective_fft_size);
                        }
                }
            // Compute the test statistic
            if (d_use_CFAR_algorithm_flag)
    ...
    if ((d_num_noncoherent_integrations_counter == acq_parameters.max_dwells) or (d_positive_acq == 1))
        {
            ...
            if (d_dump and d_channel == d_dump_channel)
                {
                    pcps_noncoherent_iq_acquisition_caf_cc::dump_results(effective_fft_size);
                }
            d_num_noncoherent_integrations_counter = 0U;
            d_positive_acq = 0;
            // Reset grid
...
bool pcps_noncoherent_iq_acquisition_caf_cc::start()
{
...
int pcps_noncoherent_iq_acquisition_caf_cc::general_work(int noutput_items __attribute__((unused)),

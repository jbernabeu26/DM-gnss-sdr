...
{
	std::vector <int32_t> legendre(WEIL_N);
	for(int i=0;i<WEIL_N;i++)
	{
		legendre[i]=0;
	}
	for(int k=1;k<WEIL_N;k++)
	{
		int leg = 0;
		for(int x=1;x<WEIL_N;x++)
		{
			int power= pow(x,2);
			if(( power % WEIL_N) == k)
			{	
...
std::vector < int > generate_legendre_sequence_secondary()
{
	...
	for(int i=0;i<WEIL_N_SECONDARY;i++)
	{
		legendre[i]=0;
	}
	for(int k=1;k<WEIL_N_SECONDARY;k++)
	{
		int leg = 0;
		for(int x=1;x<WEIL_N_SECONDARY;x++)
		{
			int power= pow(x,2);
			if(( power % WEIL_N_SECONDARY) == k)
			{	
				leg = 1;
				break;
			}
...
{	
	...
	std::vector<int32_t> legendre1(WEIL_N);
...
void make_b1cp_secondary(int32_t* _dest, int prn)
{
	...
	//creating new vector for rotation	
	std::vector<int32_t> legendre1(WEIL_N_SECONDARY);
	//copying the elements of legendre in legendre1 for computing weil 
	legendre1 = legendre;
	...
	int32_t weil[WEIL_N_SECONDARY];
	for(int i=0;i<WEIL_N_SECONDARY;i++)
	{	
		weil[i]=legendre[i] xor legendre1[i];
	}
	//copying the elements of weil in weil1 for computing weil_rot
	std::vector<int32_t> weil1(weil,weil+(WEIL_N_SECONDARY));
	int32_t rotL2= BEIDOU_B1cp_Secondary_Trunc_Point[prn-1]- 1;
	std::rotate(weil1.begin(),weil1.begin()+rotL2,weil1.end());
	
	int32_t weil_rot[WEIL_N_SECONDARY];
	for (int i=0; i < WEIL_N_SECONDARY; i++)
	{ 
		weil_rot[i]=weil1[i];
	}

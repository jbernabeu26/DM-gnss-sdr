/*!
 * \file beidou_b1c_signal_processing_new.cc
...
void beidou_b1c_data_sinboc_11_gen_int(int* _dest, const int* _prn, uint32_t _length_out)
{
...
void beidou_b1c_pilot_sinboc_11_gen_int(int* _dest, const int* _prn, uint32_t _length_out)
{
...
void beidou_b1c_pilot_sinboc_61_gen_int(int* _dest, const int* _prn, uint32_t _length_out)
{
    const uint32_t _length_in = BEIDOU_B1Cp_CODE_LENGTH_CHIPS;
    auto _period = static_cast<uint32_t>(_length_out / _length_in);
...
void beidou_b1c_gen_float(float* _dest, int* _prn, char _Signal[10])
{
    ...
    const float gamma = sqrt(29.0 / 44.0);
    
    
    int32_t sinboc_11_data[12 * 10230] = {0};  //  _codeLength not accepted by Clang
    int32_t sinboc_11_pilot[12 * 10230] = {0};
    int32_t sinboc_61_pilot[12 * 10230] = {0};


    beidou_b1c_data_sinboc_11_gen_int(sinboc_11_data, _prn, _codeLength);  //generate sinboc(1,1) 12 samples per chip
    beidou_b1c_pilot_sinboc_11_gen_int(sinboc_11_pilot, _prn, _codeLength);  //generate sinboc(1,1) 12 samples per chip
    beidou_b1c_pilot_sinboc_61_gen_int(sinboc_61_pilot, _prn, _codeLength);  //generate sinboc(6,1) 12 samples per chip
    
    //1B for Data and 1C for Pilot Code
    if (_beidou_signal.rfind("1B") != std::string::npos && _beidou_signal.length() >= 2)
        {
            for (uint32_t i = 0; i < _codeLength; i++)
                {
                    _dest[i] = alpha * static_cast<float>(sinboc_11_data[i])
                              
                }
        }
    else if (_beidou_signal.rfind("1C") != std::string::npos && _beidou_signal.length() >= 2)
        {
            for (uint32_t i = 0; i < _codeLength; i++)
                {
                    _dest[i] = beta * static_cast<float>(sinboc_61_pilot[i]) - gamma * static_cast<float>(sinboc_11_pilot[i]);
                }
        }
}

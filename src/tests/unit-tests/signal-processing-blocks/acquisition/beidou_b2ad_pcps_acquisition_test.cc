...
void BeidouB2adPcpsAcquisitionTest::init()
{
    ...
    gnss_synchro.PRN = 27;
    config->set_property("GNSS-SDR.internal_fs_sps", std::to_string(fs));
...
void BeidouB2adPcpsAcquisitionTest::plot_grid()
{
    //load the measured values
    std::string basename = "./tmp-acq-bds-b2a/acquisition";
    unsigned int sat = static_cast<unsigned int>(gnss_synchro.PRN);

    unsigned int samples_per_code = static_cast<unsigned int>(round(fs / (BEIDOU_B2ad_CODE_RATE_HZ / BEIDOU_B2ad_CODE_LENGTH_CHIPS)));  // !!
            ...
            try
                {
                    ...
                    g1.set_title("BDS B2a signal acquisition for PRN #" + std::to_string(gnss_synchro.PRN));
